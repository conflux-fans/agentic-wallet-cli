...
export function formatTransactionPlan(plan: TransactionPlan): string {
  if (plan.kind === "contract-call") {
    return [
      ...
      `合约：${plan.to}`,
      ...plan.summary,
      `Value：${formatNativeTransactionAmount(plan.nativeSymbol, plan.nativeDecimals, plan.value)}`,
      `预估 gas：${plan.estimatedGas.toString()}`,
      `预估费用：${formatNativeTransactionAmount(plan.nativeSymbol, plan.nativeDecimals, plan.estimatedFee)}`,
      "",
...
function formatNativeTransactionAmount(symbol: string, decimals: number, value: bigint): string {
  return `${padNativeDecimals(formatUnits(value, decimals))} ${symbol}`;
}
